...
#ifndef VMT_LON_LAT_CELL_LOCATOR
#define VMT_LON_LAT_CELL_LOCATOR

/**
 * A class to quickly find a cell in a (lon, lat[, elev=0]) unstructured grid,
 * periodic in longitude and (optionally) folding across the poles -- this is
 * the periodicity/pole-aware implementation of vmtCellLocator's interface.
 *
 * @note for a genuinely 3D-embedded (x, y, z) mesh with no periodic seam --
 *       e.g. real Cartesian surface coordinates rather than (lon, lat) --
 *       use vmtXYZCellLocator instead: this class's bucket search bins
 *       points using only their first two (x, y) coordinates (a valid
 *       simplification for a lon-lat chart, but not for one where the 3rd
 *       coordinate genuinely varies), and its containment/interpolation math
 *       assumes points are (lon, lat) in degrees wherever periodX > 0.
 */
class vmtLonLatCellLocator : public vmtCellLocator {

public:

    static vmtLonLatCellLocator* New() {
        return new vmtLonLatCellLocator();
    }

    /**
     * Constructor
     */
    vmtLonLatCellLocator();
     ...
     */
    vtkIdType FindCell(const double point[3], double tol2, vtkGenericCell *cell, double pcoords[3], double *weights) override;
...
private:
     ...
     * @note bins on (x, y) only -- valid for a (lon, lat[, elev=0]) chart, NOT
     *       for a genuinely 3D-embedded surface (see vmtXYZCellLocator instead)
     */
...
#endif // VMT_LON_LAT_CELL_LOCATOR
