/*=========================================================================
  ...
  Module:    mvtkCellLocator.cxx (modified by Alex Pletzer)
...
vtkStandardNewMacro(mvtkCellLocator);
...
// 25 cells per bucket.
mvtkCellLocator::mvtkCellLocator()
{
  this->MaxLevel             = 8;
  this->Level                = 8;
...
//----------------------------------------------------------------------------
void mvtkCellLocator::FreeSearchStructure()
{
  vtkIdList *cellIds;
  int i;
...
int mvtkCellLocator::GenerateIndex(int offset, int numDivs, int i, int j,
                                  int k, vtkIdType &idx)
{
  if ( i < 0 || i >= numDivs ||
...
//----------------------------------------------------------------------------
void mvtkCellLocator::ComputeOctantBounds(int i, int j, int k)
{
...
// finite line
int mvtkCellLocator::IntersectWithLine(double a0[3], double a1[3], double tol,
...
void mvtkCellLocator::FindClosestPoint(double x[3], double closestPoint[3],
...
//----------------------------------------------------------------------------
vtkIdType mvtkCellLocator::FindClosestPointWithinRadius(double x[3], double radius,
...
//
void mvtkCellLocator::GetBucketNeighbors(int ijk[3], int ndivs, int level)
{
...
// in the bucket list.
//
void mvtkCellLocator::GetOverlappingBuckets(double x[3], int vtkNotUsed(ijk)[3],
                                           double dist,
                                           int prevMinLevel[3],
                                           int prevMaxLevel[3])
{
...
int mvtkCellLocator::GetNumberOfBuckets(void)
{
  if (this->Tree)
  {
...
vtkIdList* mvtkCellLocator::GetCells(int octantId)
{
  // handle parents ?
...
//---------------------------------------------------------------------------
void mvtkCellLocator::BuildLocator()
{
  if (this->LazyEvaluation) return;
  this->ForceBuildLocator();
}
//---------------------------------------------------------------------------
void mvtkCellLocator::BuildLocatorIfNeeded()
{
...
//---------------------------------------------------------------------------
void mvtkCellLocator::ForceBuildLocator()
{
  //
...
//
void mvtkCellLocator::BuildLocatorInternal()
{
...
//----------------------------------------------------------------------------
void mvtkCellLocator::MarkParents(void* a, int i, int j, int k,
                                 int ndivs, int level)
{
  int offset, prod, ii;
  vtkIdType parentIdx;
...
//----------------------------------------------------------------------------
void mvtkCellLocator::GenerateRepresentation(int level, vtkPolyData *pd)
{
  vtkPoints *pts;
  vtkCellArray *polys;
...
//----------------------------------------------------------------------------
void mvtkCellLocator::GenerateFace(int face, int numDivs, int i, int j, int k,
...
//----------------------------------------------------------------------------
void mvtkCellLocator::ClearCellHasBeenVisited()
{
...
//----------------------------------------------------------------------------
void mvtkCellLocator::ClearCellHasBeenVisited(int id)
{
...
//
double mvtkCellLocator::Distance2ToBucket(double x[3], int nei[3])
{
  double bounds[6];
...
double mvtkCellLocator::Distance2ToBounds(double x[3], double bounds[6])
{
  double distance;
  double deltas[3];
...
//----------------------------------------------------------------------------
static bool mvtkCellLocator_Inside(const double bounds[6], const double point[3]) {
...
//----------------------------------------------------------------------------
vtkIdType mvtkCellLocator::FindCell(
  ...
  double pcoords[3], double *weights)
{
      ...
      ijk[2]*this->NumberOfDivisions*this->NumberOfDivisions]) != nullptr )
  {
    ...
    for (int j=0; j < cellIds->GetNumberOfIds(); j++)
    {
      ...
      {
        this->DataSet->GetCellBounds(cellId, cellBounds);
        if (mvtkCellLocator_Inside(cellBounds, x))
        {
          this->DataSet->GetCell(cellId, cell);
          if (cell->EvaluatePosition(x, nullptr, subId, pcoords, dist2, weights)==1)
          {
...
//----------------------------------------------------------------------------
void mvtkCellLocator::FindCellsWithinBounds(double *bbox, vtkIdList *cells)
{
  this->BuildLocatorIfNeeded();
...
//----------------------------------------------------------------------------
void mvtkCellLocator::FindCellsAlongLine(double p1[3], double p2[3], double vtkNotUsed(tol),
                                        vtkIdList *cells)
{
  this->BuildLocatorIfNeeded();
  ...
  tMax = sqrt(tMax);
  std::cerr << "*** tMax = " << tMax << " direction2 = " << direction2[0] << ',' << direction2[1] << ',' << direction2[2] << '\n';
...
//----------------------------------------------------------------------------
void mvtkCellLocator::PrintSelf(ostream& os, vtkIndent indent)
{
