...
def test_vectorRegridW2():
    ...
    for icell in range(src_numCells):
        for ie in range(NUM_VERTS_PER_QUAD):
            ...
            src_u[edgeId] = - numpy.sin(theMid) * numpy.cos(lamMid)
            src_v[edgeId] = numpy.sin(lamMid)

    # regrid
    rg.vectorApply(src_u, src_v, dst_u, dst_v, FUNC_SPACE_W2)
    ...
    for icell in range(dst_numCells):
        for ie in range(NUM_VERTS_PER_QUAD):
            ...
            dst_uExact = - numpy.sin(theMid) * numpy.cos(lamMid)
            dst_vExact = numpy.sin(lamMid)
...
def test_fv3_to_latlon_weights():
    ...
    for icell in range(src_ncells):
        # iterate nodes in CCW order, compute φ(B) − φ(A) for each edge
        for ie in range(NUM_EDGES_PER_QUAD):
            ...
            # reverse the sign for the top and left edges to match the MINT edge orientation
            sign = -2*(ie // 2) + 1
